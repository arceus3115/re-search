...
async def search_papers(
    ...
    """
    logger.info(
        f"GET /api/v1/search request received with search_term: {search_term}, from_year: {from_year}, country_code: {country_code}, topic_ids: {topic_ids}, page: {page}, per_page: {per_page}"
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched {len(works)} works for search term: {search_term}"
        )
...
@app.get("/api/v1/crawl_universities")
async def trigger_university_crawl():
    """Triggers the university crawling process.

    Returns:
        dict: A message indicating the crawling process has been initiated or an error occurred.
    """
    logger.info("GET /api/v1/crawl_universities request received.")
    try:
        crawled_data = await crawl_universities()
        logger.info("University crawling completed successfully.")
        return {"crawled_data": crawled_data}
    except Exception as e:
        logger.error(f"Error during university crawling: {e}")
        return {"message": f"Error during university crawling: {e}"}, 500
